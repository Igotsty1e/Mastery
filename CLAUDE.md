...
- `/Users/ivankhanaev/Mastery/docs/design-mockups/` is the canonical **visual reference** for screen-level composition. Eight mobile mockups (390×844): home onboarding/dashboard, lesson intro, three text-based exercise types (multiple choice, fill-the-blank, sentence correction), result, summary. Built directly against DESIGN.md tokens with real lesson content from `backend/data/lessons/b2-lesson-001.json`. The `listening_discrimination` exercise variant is specified in `DESIGN.md §14` (not in the mockup set). In-flight visual explorations live under subdirectories — currently `docs/design-mockups/onboarding-2step/` for the 2-step onboarding + dashboard-as-home wave.
- `/Users/ivankhanaev/Mastery/docs/plans/arrival-ritual.md` is the approved screen contract for the onboarding + first-exercise V2 wave. **Active direction:** 2-step onboarding (`Promise` → `Assembly`) ending in a dashboard that doubles as the single Home (also the destination of `Done` from SummaryScreen). The original 3-step direction shipped briefly on 2026-04-26 (`cea886f..bd0f021`) and was replaced the same day; that code is still in `app/lib/screens/onboarding_arrival_ritual_screen.dart` and will be replaced by the implementation that follows the in-flight design-shotgun output. Use this doc together with `DESIGN.md` before changing those flows.
