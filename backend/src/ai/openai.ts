...
export class OpenAiProvider implements AiProvider {
  ...
  async evaluateFreeSentence(
    ...
    const prompt = [
      ...
      ``,
      `Both conditions MUST be true to mark correct=true:`,
      `  (a) the sentence is grammatically well-formed standard English, AND`,
      `  (b) the sentence demonstrably APPLIES the target rule — uses the`,
      `      structure / verb / form the rule names. If the rule says`,
      `      "after enjoy, use -ing", a sentence with "want to swim" or`,
      `      "I am happy" is correct=false even if it parses, because it`,
      `      does not USE the rule. The instruction usually names the`,
      `      trigger verb / structure the sentence must contain — read it.`,
      ``,
      `Return strict JSON matching the provided schema.`,
      `- correct: true only if BOTH (a) AND (b) above. Otherwise false.`,
      `- feedback: short (<= 80 chars). When correct=false, name the`,
      `  specific failure ("no -ing form after 'enjoy'" / "uses 'to + inf'"`,
      `  / "ungrammatical: missing subject"). Empty string is allowed`,
      `  on correct=true.`,
      ``,
      `Examples of how the gates apply (do NOT echo these to the user):`,
      `  rule="after enjoy, the next verb takes -ing":`,
      `    "I enjoy reading on weekends." → correct=true`,
      `    "I enjoy to read on weekends." → correct=false (uses to+inf)`,
      `    "I want to swim on weekends." → correct=false (does not use enjoy)`,
      `    "Me reading books and enjoy them yes" → correct=false (ungrammatical)`,
      ``,
      `Treat 1-2 character spelling typos in a single word as acceptable`,
      `when the intended word is unambiguous (e.g. "fo" for "for", "enjoi"`,
      `for "enjoy"). Do NOT treat number, inflection, or determiner`,
      `changes as typos.`,
      ``,
