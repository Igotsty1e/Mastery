...
|--------|-------|-------------|
| HomeScreen | `/` | First launch: 2-step Arrival Ritual onboarding (`Promise` → `Assembly`) per Direction A · Editorial Notebook in `docs/plans/arrival-ritual.md`. Final CTA reveals the dashboard. Returning launch and post-summary `Done` also land on the dashboard (level selector + progress card). State branched on `onboarding_arrival_ritual_seen_v2`. |
