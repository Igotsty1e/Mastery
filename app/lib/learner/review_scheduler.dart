...
///
/// Wave 3 V0: device-scoped SharedPreferences, mirroring `LearnerSkillStore`.
/// Server-side review state is a follow-up wave once accounts exist.
///
...
///
/// State transitions:
/// - First time a skill reaches `almost_mastered`/`mastered`, enter the
///   cadence at step 1.
/// - A correct review attempt advances to the next step.
/// - A wrong review attempt (or a 3rd-mistake `weak` outcome per §9.1)
///   resets the skill to step 1.
/// - After step 4 cleared without resetting, the skill may be `graduated`
///   (per §9.4 — soft signal, not a guarantee).
class ReviewScheduler {
  ...
  /// Records the in-session outcome for one skill at session end. The
  /// scheduler computes the next due time and stores it.
  ///
  /// `mistakesInSession` is the count of wrong attempts on this skill
  /// during the just-finished session. The §9.1 3rd-mistake `weak`
  /// outcome resets the cadence to step 1.
  static Future<ReviewSchedule?> recordSessionEnd({
    ...
    try {
      ...
      final dueAt = now.add(intervalForStep(nextStep));
      ...
      // Tolerate persistence failure — the lesson flow keeps working.
      return null;
    }
  }

  /// Returns every skill whose review is due at or before `now`. Sorted
  /// by `dueAt` (oldest first) so the dashboard can surface the most
  /// overdue review at the top.
  static Future<List<ReviewSchedule>> dueAt(DateTime now) async {
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<ReviewSchedule?> get(String skillId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  /// Returns every schedule entry the learner has on this device.
  static Future<List<ReviewSchedule>> all() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final ids = prefs.getStringList(_indexKey) ?? const <String>[];
      final out = <ReviewSchedule>[];
  ...
  /// Test-only: clears every schedule entry and the index.
