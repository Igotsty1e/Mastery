...
| [`plans/arrival-ritual.md`](plans/arrival-ritual.md) | Onboarding (Direction A · Editorial Notebook) shipped 2026-04-26: 2-step + dashboard-as-home, calm route transitions, code-level conformance audit. First-exercise V2 chrome redesign declined by the product owner — current exercise chrome is the long-term contract. Live visual QA in a real browser still pending. |
| [`plans/dashboard-study-desk.md`](plans/dashboard-study-desk.md) | Dashboard V2 (Study Desk) shipped 2026-04-26, then heavily reduced by Wave 0 automaticity pivot (2026-05-01) and Wave 11.4 (2026-04-28). Wave 0 retired header chrome (eyebrow + greeting + sub + `M` avatar), collapsed Rules card to `_RulesTrigger`, and dropped the Last-lesson-report block. Wave 11.4 retired the Current-unit card once the V1 dynamic flow assembled each session from the skill bank. Current shipped order: level dropdown → next-lesson hero → optional ReviewDueSection → RulesTrigger → premium stub. The `/dashboard.last_lesson_report` rebind that was tracked as tech debt is moot — surface no longer exists. |
...
| [`plans/wave4-transparency-layer.md`](plans/wave4-transparency-layer.md) | Wave 4 — Transparency Layer (LEARNING_ENGINE.md §11). Engine plumbing already shipped in Wave 3 (DecisionEngine reasons, LearnerSkillStore status, ReviewScheduler.dueAt); this doc scopes the per-skill / per-routing UI surfaces that consume it. Draft, awaiting scope approval. |
| [`plans/automaticity-pivot.md`](plans/automaticity-pivot.md) | **Active live roadmap** for the automaticity pivot. Tracks Waves 0 / A / B / C / D / F / G1–G9 / H1 / H2 / H3 (phases 1 + 2 + 3) shipped state and the planned next steps (Wave E diagnostic redesign, latency-driven engine tuning). Wave H2 phase 2 investigated and deferred per the architectural blocker recorded inline. Update in lockstep with each shipped wave. |
| [`plans/postgres-vps-cutover.md`](plans/postgres-vps-cutover.md) | **Operator runbook** for moving Mastery's Postgres off the Render free tier (auto-deletes 2026-05-27) onto the LevelChannel Timeweb VPS. Recommends co-locating the backend on the same VPS (Option B) to avoid public-Postgres exposure + cross-cloud latency. Includes pre-flight checklist, full cutover sequence, smoke tests, rollback procedure, and post-cutover follow-ups. Drafted 2026-05-14; cutover window opens 2026-05-20. |
