...
- **Wave 14.2 phase 2 (2026-04-28)** — V1.5 open-answer authoring sprint + runtime-supported-types gate. 10 `sentence_rewrite` items added to the bank (2 per skill across all 5 shipped B2 skills), authored via the `english-grammar-methodologist` skill against Swan PEU + Murphy EGiU. Distribution: 4 `form_error` / 2 `contrast_error` / 4 `conceptual_error`; 4 of 10 marked `strongest` with `meaning_frame`. Each lesson grew 10 → 12 items in the source JSON; manifest counts updated. **Server-side gate**: `RUNTIME_SUPPORTED_EXERCISE_TYPES` in `exerciseBank.ts` whitelists the renderable types — `sentence_rewrite` is intentionally NOT in it yet, so the Decision Engine cannot pick those items. Items are still indexed by `getBankEntry(id)` so the answer-time lookup works once the gate flips. Phase 3 turns it on by adding the type to the set in lockstep with the Flutter widget.
