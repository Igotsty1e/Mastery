...
- Real auth (still on `google_stub`). **Target platform refocused 2026-05-14 — first store ship is Google Play, not App Store.** Google Play submission is in flight; final OK pending. Real auth on the Android channel lands alongside the submission. Google Sign-In + iOS build are deferred until after Android is open. Until real auth lands on any channel, every signed-in user is a stub user, so:
  ...
  - **Resume work (Android-first):** finalise Google Play data-safety + content-rating forms → register the package id + chosen auth method (Google Sign-In via `id_token` verification is the natural default for Android; alternative paths can be picked when the Android wiring starts) → swap `/auth/google/stub/login` for the chosen real verifier route (verifies the provider's identity token against the public JWKS, extracts `sub` for the existing identity model). Stub route stays behind `GOOGLE_STUB_ENABLED` for staging smoke tests + the web build. Same-session validation steps: set `ADMIN_USER_IDS=<founder users.id>` on Render, sign up a fresh user via the real Android flow, confirm a row appears in `/admin/retention` with `cohort_size = 1`.
  - **iOS / Google Sign-In** stays parked until after Android is live. The Wave 14.3 / 14.1 gating bullets above are platform-agnostic — first real auth on ANY channel unblocks them.
