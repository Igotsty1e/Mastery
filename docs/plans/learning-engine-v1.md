...
- Real auth (still on `apple_stub`). **Target platform refocused 2026-05-14 — first store ship is Google Play, not App Store.** Google Play submission is in flight; final OK pending. Real auth on the Android channel lands alongside the submission. Apple Sign-In + iOS build are deferred until after Android is open. Until real auth lands on any channel, every signed-in user is a stub user, so:
