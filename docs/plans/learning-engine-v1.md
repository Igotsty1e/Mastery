...
- Open-answer exercise widgets (`sentence_rewrite`, `short_free_sentence`) + AI evaluator with deterministic-first → AI fallback. **In progress 2026-04-28:** phases 1-3 shipped for `sentence_rewrite` (backend foundation, 10 authored items in the bank, Flutter `SentenceRewriteWidget` + runtime gate flipped). Phase 4 still open: `short_free_sentence` needs a different evaluator (rule-conformance, not match-to-accepted) and a different AI prompt template — separate wave when we want to expand production coverage further.
...
- Feedback system surface (after-friction prompt, after-summary prompt, cooldown). **In progress 2026-04-28:** Wave 14.3 phase 1 shipped backend foundation — `feedback_responses` table, `POST /me/feedback`, `GET /me/feedback/cooldown`, 24h per-prompt-kind cooldown. Phase 2 = Flutter UI for the `after_summary` surface (modal sheet on SummaryScreen). Phase 3 = `after_friction` mid-session prompt (more nuanced trigger logic). Live signal deferred until real Apple Sign-In ships (same gating as Wave 14.1 retention).
