...
- Feedback system surface (after-friction prompt, after-summary prompt, cooldown). **In progress 2026-04-28:** phases 1-2 shipped — backend foundation (`feedback_responses` table, `POST /me/feedback`, `GET /me/feedback/cooldown`, 24h per-prompt-kind cooldown) + Flutter `after_summary` modal sheet (5-star rating + optional comment, intercepts SummaryScreen Done tap). Phase 3 (`after_friction` mid-session prompt) still open. Live signal arrives the day real Apple Sign-In ships (same gating as Wave 14.1 retention).
