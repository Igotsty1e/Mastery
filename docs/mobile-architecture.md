...
- **Wave H3 (2026-05-01) — `prominent` mode.** For `short_free_sentence` items the instruction IS the question, so the band drops its chrome and renders the text as a prominent `Manrope` w600 20px headline above the answer field. The page reads as a real question (`"What do you really enjoy doing when you travel?"`), not a meta-label. Implementation: `InstructionBand(prominent: exercise.type == ExerciseType.shortFreeSentence)` in `lib/widgets/mastery_widgets.dart`. The headline TextStyle is built directly with `fontFamily: 'Manrope'` + named weight (no `FontVariation` axes) because the variable-axis path mis-renders at w700 on Brave/Chromium and falls back to a serif (fixed by `533140d`).
...
- **Wave H3 phase 1 (2026-05-01).** Question-driven framing on Lesson 1 (TTT — Test-Teach-Test, methodologist verdict). 4 of 5 fill_blank items converted to `short_free_sentence` with question instructions; lone fill_blank …037 kept for fast recognition. Client side: new `prominent` mode on `InstructionBand` renders the `short_free_sentence` instruction as a 20-px Manrope-w600 headline (no card chrome, no icon) so the question reads as the page header. The `TextStyle` is built directly with `fontFamily: 'Manrope'` + named weight (no `FontVariation`) because the variable-axis path mis-rendered at w700 on Brave/Chromium and fell back to a generic serif. Phases 2 & 3 (Lessons 3, 5, partial 4) are documented in `docs/plans/automaticity-pivot.md` and not yet shipped.
- **Body letter-spacing (2026-05-01).** `MasteryTextStyles.bodyLg` / `bodyMd` / `bodySm` / `titleSm` carry small positive tracking (`+0.1` / `+0.15` / `+0.2` / `+0.05`) instead of zero. Manrope at zero tracking visually fuses on phones (user report); positive tracking restores legibility without changing the font. Listed under DESIGN.md §Type Scale as the binding contract.
