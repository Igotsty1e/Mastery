...
- **Wave H1 (2026-05-01) — `RuleCardView`.** When the lesson
  carries a structured `rule_card` field
  (`docs/content-contract.md §1.2`), the screen renders the
  textbook-style `RuleCardView`
  (`app/lib/widgets/rule_card.dart`): title plate, ✓ examples,
  multi-column pattern lists, `Watch out!` callouts. When
  `rule_card` is null, the screen falls back to the legacy
  flat-string parser (`_RuleCard` blocks per the rule
  below). Both paths are kept so older content keeps rendering.
- Legacy fallback: `intro_rule` is rendered as a series of
  `_RuleCard` blocks. Paragraphs whose first line is a
  short header (`Important`, `Form`, `Use`, etc.) receive
  distinct background tints, border colors, and icons to create
  visual hierarchy. Plain paragraphs render as neutral cards.
...
| `sentence_rewrite` | Multi-line text field; original sentence shown above |
...
## Post-MVP wave additions (Wave 12.5 → 14)
...
- **Wave 14 (2026-04-28).** V1.5 dashboard skill-progress UI. Each Rules-card row now renders a `SkillStatusBadge` (`app/lib/widgets/skill_status_badge.dart`) before the CEFR chip when the learner has a record for the skill. Background and foreground colors are status-coded (`mastered` → success, `reviewDue` → error, etc.); status copy is shared with the post-lesson `SkillStateCard` via the public `statusCopyFor` helper so dashboard and summary read the same labels. `_HomeScreenState` fans out `LearnerSkillStore.allRecords()` in parallel with the lessons / review-due fetches and stores them as `Map<String, LearnerSkillRecord>` keyed by `skillId` for O(1) lookup per row.
- **Wave 14 redesign (2026-05-01).** `SkillStatusBadge` was a soft-tinted pill that visually fused with the calm beige/sage chrome — observed user-side that "the per-skill state got lost". Reshaped to a 5-dot progress strip plus a sentence-case label; filled-dot count maps 1:1 to the mastery progression (`started=1/5 → mastered=5/5`); `mastered` paints all five in `success` (sage); `review_due` swaps the dots for an `errorSoft` filled circle with a `priority_high` glyph so the regressed state never reads as "back to zero". DESIGN.md §5.2 carries the visual contract. Same widget signature (`record` + `now` + optional `showLabel`); the seam is internal so callers (`_RulesRow` on the dashboard, `_SkillRow` on the summary skill panel) didn't change.
- **Wave 14.2 phase 3 (2026-04-28).** V1.5 open-answer family — Flutter `SentenceRewriteWidget` (`app/lib/widgets/sentence_rewrite_widget.dart`). Visual contract mirrors `SentenceCorrectionWidget` (ORIGINAL reference card + free-text input). Two intentional differences: input starts empty (sentence_rewrite is fresh-write, not edit-in-place), label reads "Your rewrite". `ExerciseType.sentenceRewrite` added to the model enum + parsed by `_parseType`; the `_ExerciseBody` switch in `ExerciseScreen` dispatches to the new widget. Engine-side gate flipped in the same PR (`RUNTIME_SUPPORTED_EXERCISE_TYPES` server-side now includes the type) so the 10 phase-2 items become eligible for dynamic-session selection.
- **Wave 14.4 (2026-04-28).** V1.5 open-answer family — `ShortFreeSentenceWidget` (`app/lib/widgets/short_free_sentence_widget.dart`). Bare free-text input (no ORIGINAL anchor card — anchors would bias mimicry). `ExerciseType.shortFreeSentence` added to the model enum + parsed by `_parseType`; `ExerciseScreen._ExerciseBody` switch dispatches; InstructionBand reuses the `Icons.edit_outlined` branch. Server-side gate flipped (`RUNTIME_SUPPORTED_EXERCISE_TYPES` now contains all six types).
- **Wave 14.3 phase 3 (2026-04-28).** V1.5 `after_friction` mid-session prompt. `EvaluateResponse.frictionEvent` parses the new `friction_event` field on `/answers`; `ExerciseScreen` intercepts the Next/Finish tap with `_onAdvanceTap`. When `friction_event` is set AND `getFeedbackCooldown().afterFrictionAllowed` is true, the parametrised `FeedbackPromptSheet` opens with title "How did that feel?" / subtitle naming the friction signal. Result mirrors as one `POST /me/feedback` row carrying `friction_event` + `exercise_id` in context. Failures are silent — the advance always happens.
- **Wave 14.3 phase 2 (2026-04-28).** V1.5 feedback `after_summary` UI. New `FeedbackPromptSheet` (`app/lib/widgets/feedback_prompt_sheet.dart`) — modal bottom sheet (5 tappable stars + optional 500-char comment + Send / Skip). The SummaryScreen Done tap goes through `_onDoneTap`: reads `ApiClient.getFeedbackCooldown()`, opens the sheet only when `after_summary_allowed` is true, then `submitFeedback(...)`s a single row (whatever the outcome) before popping back to the dashboard. Network failures are silent so the dashboard return is never blocked. New `FeedbackCooldown` DTO + two methods on `ApiClient` keep the dependency graph minimal — no separate feedback client.
- **Wave H1 (2026-05-01).** Textbook-style `RuleCardView` widget (`app/lib/widgets/rule_card.dart`) + plain-data `RuleCardData` model (`app/lib/models/rule_card.dart`). Threaded through `Lesson.ruleCard`, `SkillCatalogEntry.ruleCard`, and `SkillRuleSnapshot.ruleCard`. Three render sites prefer the structured card over the legacy flat `intro_rule` parser when present: `LessonIntroScreen`, dashboard `_RuleSheetBody`, and the result-panel `See full rule →` sheet inside `ResultPanel`. Visual contract: title plate + ✓ examples + multi-column pattern lists (2/3/4 cols by item count) + `Watch out!` callouts. Backend pass-through on `GET /lessons/:id`, `GET /skills`, `GET /skills/:id`, and `skill_rule_snapshot` inside `POST /lesson-sessions/:sid/answers`.
- **Wave H2 (2026-05-01).** Dual-verdict AI judge surfaced server-side; the client just consumes the existing `correct` + `explanation` fields. When the judge flips a `fill_blank` to correct because of an off-target slip, the response carries the judge's note in `explanation` so `ResultPanel` shows `"form is right; small slip on …"` rather than a silent green tick.
- **Wave H3 phase 1 (2026-05-01).** Question-driven framing on Lesson 1 (TTT — Test-Teach-Test, methodologist verdict). 4 of 5 fill_blank items converted to `short_free_sentence` with question instructions; lone fill_blank …037 kept for fast recognition. Client side: new `prominent` mode on `InstructionBand` renders the `short_free_sentence` instruction as a 20-px `Inter` w600 headline (no card chrome, no icon) so the question reads as the page header. The `TextStyle` is built directly with `fontFamily: 'Inter'` + named weight (no `FontVariation`).
- **Wave H3 phase 2 (2026-05-14).** Same conversion on Lessons 3 + 5 (4 items each). Lesson 3 verbs converted: hope/plan/want/refuse — anchors include `to` to block `verb + to + NP` bypass. Lesson 5 verbs converted: like/love/hate/begin — anchors omit `to` because both `-ing` and `to + V` are correct; dual-form acceptance is encoded directly inside `target_rule` because the Wave G6 production grader prompt no longer substitutes `accepted_examples` into the prompt body. No client-side change in this phase.
- **Wave H3 phase 3 (2026-05-14).** Lesson 4 partial conversion. 4 fill_blank items (031 remember+to, 032 remember+-ing, 034 forget+-ing, 038 try+-ing) converted to short_free_sentence with the strict 5-component `target_rule` contract (trigger / immediate complement / target meaning / rejected counter-form / explicit reject clause) — Lesson 4 is meaning-contrast, so the rule itself must disambiguate the sense. The `stop` fill_blank pair (035/036) stays untouched as the recognition-contrast anchor. The retired SFS at `…03d` is dropped (not reauthored — kept the analytics clean per `observability/exerciseStats.ts`); a fresh `regret + -ing` SFS lands on a new UUID `…040`. `§6.7.2` policy now names Lessons 1/3/4/5 as production-target with a `≥ 4` SFS target; binding mechanical floor stays at `≥ 1` so Lesson 2 (named contrast exception) keeps shipping.
- **Wave F (2026-05-14).** Progressive hint stripping on `fill_blank` items. `FillBlankWidget` gains a `HintRevealMode` enum (`always` / `after4s` / `never`) and a `Timer?` driven by per-skill lifetime attempts. `SessionController.skillAttemptsAtStart` is the canonical snapshot, populated best-effort via `LearnerSkillStore.allRecords()` in both `loadDynamicSession()` (prod path) and `loadLesson()` (legacy). The hint regex `(_{2,})\s*\([a-z][^)]*\)` is anchored to the blank token and preserves the `___` marker. Empty / failed snapshot falls back to `always` — learner sees the hint, no info loss. Cycle-aware reset on `review_due` regressions is parked behind product feedback.
- **Body letter-spacing (2026-05-01).** `MasteryTextStyles.bodyLg` / `bodyMd` / `bodySm` / `titleSm` carry small positive tracking (`+0.1` / `+0.15` / `+0.2` / `+0.05`) instead of zero. Calibrated against the body-font swap to Inter on 2026-05-02; positive tracking keeps glyphs from visually fusing on phones (user report). Listed under DESIGN.md §Type Scale as the binding contract.
- **Body-font swap to Inter (2026-05-02).** `MasteryTextStyles` body / title / label / headline-md helpers and the prominent `InstructionBand` headline now render in `Inter` instead of `Manrope`. Manrope-Variable mis-rendered on Brave / Chromium (the variable `wght` axis fell back to a generic serif at w700) and stayed visually tight on small screens even with positive tracking. Inter Variable from rsms.me is shipped at `app/assets/fonts/Inter-Variable.ttf`; the `_inter` helper in `mastery_theme.dart` sets `fontFamily` + named `fontWeight` and never stamps `FontVariation`, so the same code path renders cleanly on every platform. Manrope-Variable.ttf and its OFL notice were removed from `assets/fonts/` in the same commit.
