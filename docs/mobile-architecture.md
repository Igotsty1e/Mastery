...
- **Wave H3 phase 2 (2026-05-14).** Same conversion on Lessons 3 + 5 (4 items each). Lesson 3 verbs converted: hope/plan/want/refuse — anchors include `to` to block `verb + to + NP` bypass. Lesson 5 verbs converted: like/love/hate/begin — anchors omit `to` because both `-ing` and `to + V` are correct; dual-form acceptance is encoded directly inside `target_rule` because the Wave G6 production grader prompt no longer substitutes `accepted_examples` into the prompt body. No client-side change in this phase. Phase 3 (Lesson 4 partial; Lesson 2 untouched; `§6.7` floor lift to `≥4 SFS/lesson`) remains parked in `docs/plans/automaticity-pivot.md`.
